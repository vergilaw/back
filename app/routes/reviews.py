...
from app.schemas.review import ReviewCreate, ReviewResponse, ProductRating
...
async def create_review(
    ...
    """Tạo review (User - chỉ khi đã mua sản phẩm)"""
    db = get_database()
    ...
    return ReviewModel.review_to_dict(review)


@router.get("/product/{product_id}", response_model=List[ReviewResponse])
async def get_product_reviews(product_id: str):
    """Lấy reviews của sản phẩm (Public - chỉ approved)"""
    db = get_database()
    reviews = ReviewModel.find_by_product(db, product_id, only_approved=True)
    return [ReviewModel.review_to_dict(r) for r in reviews]
